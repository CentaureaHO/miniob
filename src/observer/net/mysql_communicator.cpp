...
struct OkPacket : public BasePacket
{
    ...
    virtual RC encode(uint32_t capabilities, std::vector<char>& net_packet) const override
    {
        ...
        else if (capabilities & CLIENT_TRANSACTIONS) { pos += store_int2(buf + pos, status_flags); }

        if (capabilities & CLIENT_SESSION_TRACK) { pos += store_lenenc_string(buf + pos, info.c_str()); }
        else { pos += store_fix_length_string(buf + pos, info.c_str(), info.length()); }
...
RC MysqlCommunicator::write_state(SessionEvent* event, bool& need_disconnect)
{
    ...
    else { snprintf(buf, buf_size, "%s > %s", strrc(sql_result->return_code()), state_string.c_str()); }
...
RC MysqlCommunicator::send_column_definition(SqlResult* sql_result, bool& need_disconnect)
{
    ...
    for (int i = 0; i < cell_num; i++)
    {
        ...
        pos += 1;
        store_int2(
            buf + pos, 0);  // 按照mariadb的文档描述，最后还有一个unused字段int<2>，不过mysql的文档没有给出这样的描述
        pos += 2;
    ...
    else { LOG_TRACE("client use CLIENT_DEPRECATE_EOF"); }
