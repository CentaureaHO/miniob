    ...
    Table* table, std::vector<std::string>& file_values, std::vector<Value>& record_values, std::stringstream& errmsg)
{
    ...
    if (file_values.size() < record_values.size()) { return RC::SCHEMA_FIELD_MISSING; }
    ...
    for (int i = 0; i < field_num && RC::SUCCESS == rc; i++)
    {
        ...
        if (field->type() != CHARS) { common::strip(file_value); }
        ...
        switch (field->type())
        {
            case INTS: {
                ...
                else { record_values[i].set_int(int_value); }
            }
            ...
            case FLOATS: {
                ...
                else { record_values[i].set_float(float_value); }
            }
            break;
            case CHARS: {
    ...
    if (RC::SUCCESS == rc)
    {
        ...
        if (rc != RC::SUCCESS) { errmsg << "insert failed."; }
        else if (RC::SUCCESS != (rc = table->insert_record(record))) { errmsg << "insert failed."; }
    }
    return rc;
}
...
void LoadDataExecutor::load_data(Table* table, const char* file_name, SqlResult* sql_result)
{
    ...
    while (!fs.eof() && RC::SUCCESS == rc)
    {
        std::getline(fs, line);
        line_num++;
        if (common::is_blank(line.c_str())) { continue; }
