...
class SetVariableExecutor
{
  public:
    ...
    RC execute(SQLStageEvent* sql_event)
    {
        ...
        else { rc = RC::VARIABLE_NOT_EXISTS; }
  ...
  private:
    RC var_value_to_boolean(const Value& var_value, bool& bool_value) const
    {
        ...
        if (var_value.attr_type() == AttrType::BOOLEANS) { bool_value = var_value.get_boolean(); }
        else if (var_value.attr_type() == AttrType::INTS) { bool_value = var_value.get_int() != 0; }
        else if (var_value.attr_type() == AttrType::FLOATS) { bool_value = var_value.get_float() != 0.0; }
