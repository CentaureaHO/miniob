...
class RowTuple : public Tuple
{
  public:
    ...
    void set_schema(const Table* table, const std::vector<FieldMeta>* fields)
    {
        ...
        for (const FieldMeta& field : *fields) { speces_.push_back(new FieldExpr(table, &field)); }
    }
...
class ProjectTuple : public Tuple
{
  public:
    ...
    RC cell_at(int index, Value& cell) const override
    {
        if (index < 0 || index >= static_cast<int>(speces_.size())) { return RC::INTERNAL; }
...
class ExpressionTuple : public Tuple
{
  public:
    ...
    RC cell_at(int index, Value& cell) const override
    {
        if (index < 0 || index >= static_cast<int>(expressions_.size())) { return RC::INTERNAL; }
    ...
    RC find_cell(const TupleCellSpec& spec, Value& cell) const override
    {
        ...
        {
            if (0 == strcmp(spec.alias(), expr->name().c_str())) { return expr->try_get_value(cell); }
        }
        return RC::NOTFOUND;
    }
...
class ValueListTuple : public Tuple
{
  public:
    ...
    {
        if (index < 0 || index >= cell_num()) { return RC::NOTFOUND; }
...
class JoinedTuple : public Tuple
{
  public:
    ...
    RC cell_at(int index, Value& value) const override
    {
        ...
        if (index > 0 && index < left_cell_num) { return left_->cell_at(index, value); }
