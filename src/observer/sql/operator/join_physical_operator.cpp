...
RC NestedLoopJoinPhysicalOperator::next()
{
    ...
    RC   rc = RC::SUCCESS;
    if (round_done_) { left_need_step = true; }
    else
    {
        rc = right_next();
        if (rc != RC::SUCCESS)
        {
            if (rc == RC::RECORD_EOF) { left_need_step = true; }
            else { return rc; }
        }
        else
        {
...
{
    RC rc = left_->close();
    if (rc != RC::SUCCESS) { LOG_WARN("failed to close left oper. rc=%s", strrc(rc)); }

    if (!right_closed_)
    {
        rc = right_->close();
        if (rc != RC::SUCCESS) { LOG_WARN("failed to close right oper. rc=%s", strrc(rc)); }
        else { right_closed_ = true; }
    }
    return rc;
}
...
RC NestedLoopJoinPhysicalOperator::right_next()
{
    ...
    if (rc != RC::SUCCESS)
    {
        if (rc == RC::RECORD_EOF) { round_done_ = true; }
        return rc;
    }
