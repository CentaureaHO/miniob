...
{
    RC rc = RC::SUCCESS;
    if (oper->type() != LogicalOperatorType::PREDICATE || oper->children().size() != 1) return rc;
    ...
    else if (child_oper->type() == LogicalOperatorType::JOIN)
    {
        ...
        if (left_join_child_type == LogicalOperatorType::JOIN &&
            right_join_child_type == LogicalOperatorType::TABLE_GET)
        {
            ...
            auto it =
                std::remove_if(pushdown_exprs.begin(), pushdown_exprs.end(), [&](std::unique_ptr<Expression>& expr) {
                    auto comparison_expr        = static_cast<ComparisonExpr*>(expr.get());
                    bool related_to_right_table = 0;

                    if (comparison_expr->left()->type() == ExprType::FIELD)
                    {
                        auto left_expr         = static_cast<FieldExpr*>(comparison_expr->left().get());
                        related_to_right_table = (strcmp(left_expr->table_name(), right_table_name) == 0);
                    }
                    if (comparison_expr->right()->type() == ExprType::FIELD)
                    {
                        auto right_expr = static_cast<FieldExpr*>(comparison_expr->right().get());
                        related_to_right_table =
                            related_to_right_table || (strcmp(right_expr->table_name(), right_table_name) == 0);
                    }

                    if (related_to_right_table) right_child_exprs.push_back(std::move(expr));
                    return related_to_right_table;
                });

            pushdown_exprs.erase(it, pushdown_exprs.end());

            if (!right_child_exprs.empty())
                predicate_expr =
                    std::unique_ptr<Expression>(new ConjunctionExpr(ConjunctionExpr::Type::AND, right_child_exprs));

            if (!pushdown_exprs.empty())
            {
                std::unique_ptr<ConjunctionExpr> left_conjunction_expr(
                    new ConjunctionExpr(ConjunctionExpr::Type::AND, pushdown_exprs));
                std::unique_ptr<PredicateLogicalOperator> left_new_oper(
                    new PredicateLogicalOperator(std::move(left_conjunction_expr)));
                left_new_oper->add_child(std::move(child_oper->children()[0]));
                auto right_oper = std::move(child_oper->children()[1]);
                child_oper->children().clear();
                child_oper->add_child(std::move(left_new_oper));
                child_oper->add_child(std::move(right_oper));
            }
        }
        else if (pushdown_exprs.size() == 1)
            predicate_expr = std::move(pushdown_exprs.front());
        else if (!pushdown_exprs.empty())
            predicate_expr =
                std::unique_ptr<Expression>(new ConjunctionExpr(ConjunctionExpr::Type::AND, pushdown_exprs));
    }

    if (!predicate_expr)
    {
        LOG_TRACE("predicate_expr is nullptr");
        Value value(true);
    ...
    std::unique_ptr<Expression>& expr, std::vector<std::unique_ptr<Expression>>& pushdown_exprs)
{
    ...
    if (expr->type() == ExprType::CONJUNCTION)
    {
        ...
        // 或 操作的比较，太复杂，现在不考虑
        if (conjunction_expr->conjunction_type() == ConjunctionExpr::Type::OR) { return rc; }
        ...
        for (auto iter = child_exprs.begin(); iter != child_exprs.end();)
        {
            ...
            if (!*iter) { child_exprs.erase(iter); }
            else { ++iter; }
        }
    }
    else if (expr->type() == ExprType::COMPARISON)
    {
        ...
        if (left_expr->type() != ExprType::FIELD && right_expr->type() != ExprType::FIELD) { return rc; }
