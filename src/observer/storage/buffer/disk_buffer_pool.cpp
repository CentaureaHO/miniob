...
RC BPFrameManager::init(int pool_num)
{
    ...
    if (ret == 0) { return RC::SUCCESS; }
    return RC::NOMEM;
}

RC BPFrameManager::cleanup()
{
    if (frames_.count() > 0) { return RC::INTERNAL; }
...
RC BufferPoolIterator::init(DiskBufferPool& bp, PageNum start_page /* = 0 */)
{
    ...
    if (start_page <= 0) { current_page_num_ = 0; }
    else { current_page_num_ = start_page; }
    return RC::SUCCESS;
}
...
PageNum BufferPoolIterator::next()
{
    ...
    if (next_page != -1) { current_page_num_ = next_page; }
    return next_page;
}
...
RC DiskBufferPool::purge_page(PageNum page_num)
{
    std::scoped_lock lock_guard(lock_);
    Frame*           used_frame = frame_manager_.get(file_desc_, page_num);
    if (used_frame != nullptr) { return purge_frame(page_num, used_frame); }
...
RC DiskBufferPool::allocate_frame(PageNum page_num, Frame** buffer)
{
    auto purger = [this](Frame* frame) {
        if (!frame->dirty()) { return RC::SUCCESS; }

        RC rc = RC::SUCCESS;
        if (frame->file_desc() == file_desc_) { rc = this->flush_page_internal(*frame); }
        else { rc = bp_manager_.flush_page(*frame); }
...
{
    if (memory_size <= 0) { memory_size = MEM_POOL_ITEM_NUM * DEFAULT_ITEM_NUM_PER_POOL * BP_PAGE_SIZE; }
