...
intptr_t get_default_debug_xid()
{
  ...
  intptr_t xid = (thd == nullptr) ? 
    ...
    if (session == nullptr) { return reinterpret_cast<intptr_t>(reinterpret_cast<void*>(pthread_self())); }
    else { return reinterpret_cast<intptr_t>(session); }
}
...
void Frame::write_unlatch(intptr_t xid)
{
    ...
    if (--write_recursive_count_ == 0) { write_locker_ = 0; }
    debug_lock_.unlock();
...
{
    {
        ...
        if (1 == recursive_count) { read_lockers_.erase(xid); }
