...
CLogRecord* CLogRecord::build(const CLogRecordHeader& header, char* data)
{
    ...
    if (header.logrec_len_ <= 0) { return log_record; }
...
{
    if (nullptr == log_record) { return RC::INVALID_ARGUMENT; }

    /// total_size_ 的计算没有考虑日志头
    if (total_size_ + log_record->logrec_len() >= CLOG_BUFFER_SIZE) { return RC::LOGBUF_FULL; }
...
RC CLogRecordIterator::next()
{
    ...
    if (rc != RC::SUCCESS)
    {
        if (log_file_->eof()) { return RC::RECORD_EOF; }
