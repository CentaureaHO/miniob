...
RC DefaultConditionFilter::init(Table& table, const ConditionSqlNode& condition)
{
    ...
    if (type_left != type_right) { return RC::SCHEMA_FIELD_TYPE_MISMATCH; }
...
bool DefaultConditionFilter::filter(const Record& rec) const
{
    ...
    else { left_value.set_value(left_.value); }
    ...
    else { right_value.set_value(right_.value); }
...
bool CompositeConditionFilter::filter(const Record& rec) const
{
    ...
    {
        if (!filters_[i]->filter(rec)) { return false; }
    }
    return true;
}
