...
Table* Db::find_table(const char* table_name) const
{
    ...
    if (iter != opened_tables_.end()) { return iter->second; }
    return nullptr;
}
...
Table* Db::find_table(int32_t table_id) const
{
    ...
    {
        if (pair.second->table_id() == table_id) { return pair.second; }
    }
    return nullptr;
}
...
RC Db::open_all_tables()
{
    ...
    for (const std::string& filename : table_meta_files)
    {
        ...
        if (table->table_id() >= next_table_id_) { next_table_id_ = table->table_id() + 1; }
