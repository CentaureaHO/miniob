...
RC LeafIndexNodeHandler::move_first_to_end(LeafIndexNodeHandler& other, DiskBufferPool* disk_buffer_pool)
{
    ...
    if (size() >= 1) { memmove(__item_at(0), __item_at(1), (static_cast<size_t>(size()) - 1) * item_size()); }
    increase_size(-1);
    return RC::SUCCESS;
}
...
{
    if (size() > 0) { memmove(__item_at(1), __item_at(0), static_cast<size_t>(size()) * item_size()); }
...
bool LeafIndexNodeHandler::validate(const KeyComparator& comparator, DiskBufferPool* bp) const
{
    ...
    if (parent_page_num == BP_INVALID_PAGE_NUM) { return true; }
    ...
    int* insert_position /*= nullptr */) const
{
    ...
    if (insert_position) { *insert_position = ret; }

    if (ret >= size || comparator(key, __key_at(ret)) < 0) { return ret - 1; }
    return ret;
}
...
int InternalIndexNodeHandler::value_index(PageNum page_num)
{
    ...
    {
        if (page_num == *(PageNum*)__value_at(i)) { return i; }
    }
    return -1;
}
...
RC InternalIndexNodeHandler::preappend(const char* item, DiskBufferPool* bp)
{
    ...
    if (this->size() > 0) { memmove(__item_at(1), __item_at(0), static_cast<size_t>(this->size()) * item_size()); }
...
bool InternalIndexNodeHandler::validate(const KeyComparator& comparator, DiskBufferPool* bp) const
{
    ...
    if (parent_page_num == BP_INVALID_PAGE_NUM) { return result; }
    ...
    int leaf_max_size /* = -1 */)
{
    ...
    if (internal_max_size < 0) { internal_max_size = calc_internal_page_capacity(attr_length); }
    if (leaf_max_size < 0) { leaf_max_size = calc_leaf_page_capacity(attr_length); }
...
{
    if (disk_buffer_pool_ != nullptr) { disk_buffer_pool_->close_file(); }
...
RC BplusTreeHandler::print_internal_node_recursive(Frame* frame)
{
    ...
    for (int i = 0; i < node_size; i++)
    {
        ...
        if (node.is_leaf()) { rc = print_leaf(child_frame); }
        else { rc = print_internal_node_recursive(child_frame); }
        if (rc != RC::SUCCESS)
        {
...
RC BplusTreeHandler::print_tree()
{
    ...
    if (node.is_leaf()) { rc = print_leaf(frame); }
    else { rc = print_internal_node_recursive(frame); }
    return rc;
}
...
RC BplusTreeHandler::print_leafs()
{
    ...
    while (frame->page_num() != BP_INVALID_PAGE_NUM)
    {
        ...
        if (next_page_num == BP_INVALID_PAGE_NUM) { break; }
...
{
    // root locked
    if (op != BplusTreeOperationType::READ) { latch_memo.xlatch(&root_lock_); }
    else { latch_memo.slatch(&root_lock_); }

    if (is_empty()) { return RC::EMPTY; }
...
RC BplusTreeHandler::insert_entry_into_leaf_node(LatchMemo& latch_memo, Frame* frame, const char* key, const RID* rid)
{
    ...
    if (insert_position < leaf_node.size()) { leaf_node.insert(insert_position, key, (const char*)rid); }
    else { new_index_node.insert(insert_position - leaf_node.size(), key, (const char*)rid); }
...
RC BplusTreeHandler::get_entry(const char* user_key, int key_len, std::list<RID>& rids)
{
    ...
    scanner.close();
    if (rc != RC::RECORD_EOF) { LOG_WARN("scanner return error. rc=%s", strrc(rc)); }
    else { rc = RC::SUCCESS; }
    return rc;
}
...
RC BplusTreeHandler::coalesce_or_redistribute(LatchMemo& latch_memo, Frame* frame)
{
    ...
    if (index_node.size() >= index_node.min_size()) { return RC::SUCCESS; }
    ...
    if (index == 0) { neighbor_page_num = parent_index_node.value_at(1); }
    else { neighbor_page_num = parent_index_node.value_at(index - 1); }
    ...
    else { rc = coalesce<IndexNodeHandlerType>(latch_memo, neighbor_frame, frame, parent_frame, index); }
...
RC BplusTreeHandler::delete_entry_internal(LatchMemo& latch_memo, Frame* leaf_frame, const char* key)
{
    ...
    if (leaf_index_node.size() >= leaf_index_node.min_size()) { return RC::SUCCESS; }
    ...
    int right_len, bool right_inclusive)
{
    ...
    else
    {
        ...
        if (tree_handler_.file_header_.attr_type == CHARS)
        {
            ...
            if (should_inclusive_after_fix) { left_inclusive = true; }
        }

        MemPoolItem::unique_ptr left_pkey;
        if (left_inclusive) { left_pkey = tree_handler_.make_key(fixed_left_key, *RID::min()); }
        else { left_pkey = tree_handler_.make_key(fixed_left_key, *RID::max()); }
    ...
    if (nullptr == right_user_key) { right_key_ = nullptr; }
    else
    {
        ...
        {
            rc = fix_user_key(
                right_user_key, right_len, false /*want_greater*/, &fixed_right_key, &should_include_after_fix);
            if (rc != RC::SUCCESS)
            {
            ...
            if (should_include_after_fix) { right_inclusive = true; }
        }
        if (right_inclusive) { right_key_ = tree_handler_.make_key(fixed_right_key, *RID::max()); }
        else { right_key_ = tree_handler_.make_key(fixed_right_key, *RID::min()); }
    ...
    if (touch_end()) { current_frame_ = nullptr; }
...
{
    if (right_key_ == nullptr) { return false; }
...
{
    if (nullptr == current_frame_) { return RC::RECORD_EOF; }
    ...
    {
        if (touch_end()) { return RC::RECORD_EOF; }
    ...
    if (BP_INVALID_PAGE_NUM == next_page_num) { return RC::RECORD_EOF; }
    ...
    if (!locked) { return RC::LOCKED_NEED_WAIT; }
...
{
    if (nullptr == fixed_key || nullptr == should_inclusive) { return RC::INVALID_ARGUMENT; }
    ...
    *should_inclusive = false;
    ...
    if (nullptr == key_buf) { return RC::NOMEM; }
    ...
    *should_inclusive = true;
    if (want_greater) { key_buf[attr_length - 1]++; }
