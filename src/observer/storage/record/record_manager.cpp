...
RC RecordPageIterator::next(Record& record)
{
    ...
    if (next_slot_num_ >= 0) { next_slot_num_ = bitmap_.next_setted_bit(next_slot_num_ + 1); }
...
RC RecordPageHandler::init(DiskBufferPool& buffer_pool, PageNum page_num, bool readonly)
{
    ...
    if (readonly) { frame_->read_latch(); }
    else { frame_->write_latch(); }
...
RC RecordPageHandler::cleanup()
{
    if (disk_buffer_pool_ != nullptr)
    {
        if (readonly_) { frame_->read_unlatch(); }
        else { frame_->write_unlatch(); }
...
{
    if (nullptr == page_header_) { return (PageNum)(-1); }
...
RC RecordFileHandler::init_free_pages()
{
    ...
    while (bp_iterator.has_next())
    {
        ...
        if (!record_page_handler.is_full()) { free_pages_.insert(current_page_num); }
...
RC RecordFileScanner::fetch_next_record_in_page()
{
    ...
    while (record_page_iterator_.has_next())
    {
        ...
        // 如果有过滤条件，就用过滤条件过滤一下
        if (condition_filter_ != nullptr && !condition_filter_->filter(next_record_)) { continue; }

        // 如果是某个事务上遍历数据，还要看看事务访问是否有冲突
        if (trx_ == nullptr) { return rc; }
...
{
    if (disk_buffer_pool_ != nullptr) { disk_buffer_pool_ = nullptr; }

    if (condition_filter_ != nullptr) { condition_filter_ = nullptr; }
