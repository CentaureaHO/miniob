...
RC Table::make_record(int value_num, const Value* values, Record& record)
{
    ...
    for (int i = 0; i < value_num; i++)
    {
        ...
        if (field->type() == CHARS)
        {
            ...
            if (copy_len > data_len) { copy_len = data_len + 1; }
        }
...
RC Table::get_record_scanner(RecordFileScanner& scanner, Trx* trx, bool readonly)
{
    ...
    if (rc != RC::SUCCESS) { LOG_ERROR("failed to open scanner. rc=%s", strrc(rc)); }
    return rc;
}
...
RC Table::delete_entry_of_indexes(const char* record, const RID& rid, bool error_on_not_exists)
{
    ...
    for (Index* index : indexes_)
    {
        ...
        if (rc != RC::SUCCESS)
        {
            if (rc != RC::RECORD_INVALID_KEY || !error_on_not_exists) { break; }
        }
    }
    return rc;
}
...
Index* Table::find_index(const char* index_name) const
{
    for (Index* index : indexes_)
    {
        if (0 == strcmp(index->index_meta().name(), index_name)) { return index; }
    }
    return nullptr;
}
Index* Table::find_index_by_field(const char* field_name) const
{
    ...
    if (index_meta != nullptr) { return this->find_index(index_meta->name()); }
    return nullptr;
}
