...
MvccTrxKit::~MvccTrxKit()
{
    ...
    for (Trx* trx : tmp_trxes) { delete trx; }
}
...
Trx* MvccTrxKit::create_trx(int32_t trx_id)
{
    ...
    if (trx != nullptr)
    {
        lock_.lock();
        trxes_.push_back(trx);
        if (current_trx_id_ < trx_id) { current_trx_id_ = trx_id; }
        lock_.unlock();
    }
    return trx;
}
...
RC MvccTrx::visit_record(Table* table, Record& record, bool readonly)
{
    ...
    {
        if (trx_id_ >= begin_xid && trx_id_ <= end_xid) { rc = RC::SUCCESS; }
        else { rc = RC::RECORD_INVISIBLE; }
    }
    else if (begin_xid < 0)
    {
...
RC MvccTrx::commit_with_trx_id(int32_t commit_xid)
{
    ...
    if (!recovering_) { rc = log_manager_->commit_trx(trx_id_, commit_xid); }
...
RC MvccTrx::rollback()
{
    ...
    if (!recovering_) { rc = log_manager_->rollback_trx(trx_id_); }
